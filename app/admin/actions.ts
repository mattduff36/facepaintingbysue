...
export async function uploadPhotoAction(formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    const slot = applyUploadSlot(photos);
    ...
    if (shouldExpireCaches(true, true)) expire("gallery");
...
export async function reorderPhotosAction(orderedIds: string[]): Promise<ActionResult> {
  await requireAdmin();
  return applyPhotoPlan("reorder", undefined, (photos) => applyReorder(photos, orderedIds));
}
...
export async function applyStudioLayoutAction(plan: StudioLayoutPlan): Promise<ActionResult> {
  await requireAdmin();
  ...
  return applyPhotoPlan("studio-layout", undefined, (photos) => applyStudioLayout(photos, plan));
}
...
export async function saveSettingsAction(input: unknown): Promise<ActionResult> {
  ...
  try {
    ...
    const revision = assertCurrentRevision(parsed.value, current);
    ...
    if (shouldExpireCaches(true, true)) expire("settings");
